              ...
              <div class="row">
                <div class="col-xs-10">
                  <div class="form-group has-feedback" id="descriptionGroup">
                    <input class="form-control" placeholder="Nom dy type de salle" maxlength="255" name="name" type="text" required>
              ...
              @foreach ($typeOfRooms as $typeOfRoom)
                <tr>
                  ...
                  @if (Auth::user()->role <= 2 && Auth::user()->role > 0)
                    ...
                    <td>
                      {!! Form::model($typeOfRoom, ['route' => ['typeOfRooms.destroy', $typeOfRoom->id_room_type], 'method' => 'DELETE']) !!}
                        <a class="text-danger point-cursor submitDeleteMeal"><i class="fa fa-trash"></i></a>
                      {{ Form::close() }}
                    </td>
                  @endif
                </tr>
...
<script type="text/javascript">
  ...
  $('.submitDeleteMeal').click(function() {
    if(confirm('Voulez-vous vraiment supprimer ce type de salles ?\nToutes les salles rattachées à ce type seront supprimées !'))
      $(this).parent().submit();
  });
